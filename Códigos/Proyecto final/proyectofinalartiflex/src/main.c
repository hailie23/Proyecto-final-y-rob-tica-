...
void mqtt_task(void *pvParameters) {
    ...
    while(1) {
        // Publicar el ángulo de la rodilla en el tópico "esp_rodilla"
        sprintf(mqtt_data, "%f", global_Angulo_y1);
        esp_mqtt_client_publish(client, "esp_rodilla", mqtt_data, 0, 1, 0);

        // Publicar el ángulo del tobillo en el tópico "esp_tobillo"
        sprintf(mqtt_data, "%f", global_Angulo_y2);
        esp_mqtt_client_publish(client, "esp_tobillo", mqtt_data, 0, 1, 0);

        vTaskDelay(500 / portTICK_PERIOD_MS);  // Publicar cada medio segundo
    }
}
...
void app_main(void) {
    ...
    Angulos angulos;
    angulos.angulo_y1 = &global_Angulo_y1;
    angulos.angulo_y2 = &global_Angulo_y2;
    ...
    // Crear la tarea para leer los datos del giroscopio
    xTaskCreate(mpu_task, "mpu_task", 2048, &angulos, 5, NULL);
